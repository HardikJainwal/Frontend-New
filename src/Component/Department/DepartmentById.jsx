...
import { getFacultyByDepartment } from "../../utils/apiservice";

const DepartmentById = () => {
  const { name } = useParams();

  const { data, isLoading, error } = useQuery({
    queryKey: [QUERY_KEYS.GET_FACULTIES_BY_DEPARTMENT, name],
    queryFn: () => getFacultyByDepartment(unslugify(name)),
  });
            ...
            >
              <div className="w-28 h-28 rounded-full overflow-hidden mb-3 border">
                <img
                  src={
                    faculty.image ??
                    "https://dummyimage.com/300x300/ddd/aaa&text=Faculty"
                  }
                  alt={faculty.name}
