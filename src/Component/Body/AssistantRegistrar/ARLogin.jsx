...
const ARLogin = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      navigate("/assistantRegistrarLogin"); // ✅ fixed
    }
  }, [navigate]);
  ...
  const login = async () => {
    ...
    try {
      ...
      navigate("/assistantRegistrarDashboard");
    } catch (err) {
    ...
    <div className="p-24 flex items-center justify-center px-4">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl p-8">
        ...
        <p className="text-sm text-center text-gray-500 mt-4">
          ...
          <Link
            to="/assistantRegistrarSignUp" // ✅ fixed casing
        ...
        <p className="text-sm text-center text-gray-500 mt-6">
          Forgot password?{" "}
          <button
            onClick={() => navigate("/assistantRegistrarForgotPassword")} // ✅ fixed
            className="text-indigo-600 font-medium hover:underline"
          >
            Forgot password
          </button>
